...
export async function resolveCredential(options: CredentialOptions): Promise<string | null> {
	...
	return source ?? normalize(options.environmentValue);
}

export function describeCredentialSource(options: {
	configuredValue?: unknown;
	...
	const source = normalize(options.configuredValue);
	if (source?.startsWith("!")) return "configured command source (not resolved by status)";
	if (source) {
		if (escapedSource(source) !== null) return "configured literal (escaped)";
		const name = explicitEnvironmentName(source);
		if (name) return normalize(environment[name]) ? `present (env ${name})` : `missing (env ${name})`;
		if (isMalformedExplicitSource(source)) return "invalid credential source";
		return "configured literal (not recommended)";
	}
	return normalize(environment[options.environmentName])
		? `present (env ${options.environmentName})`
		: `missing (env ${options.environmentName})`;
}
